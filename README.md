...
```bash
git clone https://github.com/TongyiDai/dazibao.git
cd dazibao
```

将仓库交给支持 Agent Skill 的运行时后，用 `$dazibao` 触发。`SKILL.md` 是执行契约，`AGENT-GUIDE.md` 说明跨 Agent 的调用边界；飞书云文档写入仍需对应的 `lark-doc` / `lark-whiteboard` 能力和用户授权。
